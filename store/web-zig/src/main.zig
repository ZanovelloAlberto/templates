...
//
//
const a = @embedFile("frontend/main.wasm");
const indexh = @embedFile("frontend/index.html");

fn on_request(r: zap.SimpleRequest) void {
    r.setHeader("Server", "zap.example") catch unreachable;

    if (r.path) |p| {
        if (std.mem.eql(u8, p, "/main.wasm")) {
            r.setHeader("Content-Type", "application/wasm") catch unreachable;
            r.sendBody(a) catch unreachable;
            std.log.warn("{s}", .{p});
        }
    }

    r.sendBody(indexh) catch return;
}
